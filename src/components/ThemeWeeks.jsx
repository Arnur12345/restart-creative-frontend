...
const ThemeWeeks = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 py-10 px-2 md:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
          ...
          <div className="bg-white rounded-2xl shadow-lg p-8 flex items-center">
            ...
            <div>
              ...
              <div className="text-lg font-semibold text-gray-900">{lastWeek ? lastWeek.title : '-'}</div>
            </div>
          </div>
        </div>

        {/* Сетка карточек недель */}
        <h1 className="text-3xl md:text-4xl font-extrabold mb-8 text-gray-900">Тақырыптық апталар</h1>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {weeks.map(week => (
            <div
              key={week.id}
              className="rounded-2xl shadow-xl bg-white p-8 cursor-pointer border-2 border-transparent hover:border-indigo-400 transition-all duration-200 hover:scale-[1.03] flex flex-col justify-between min-h-[220px]"
              onClick={() => navigate(`/theme-weeks/${week.id}`)}
            >
              <div>
                <div className="flex items-center mb-2">
                  <span className="inline-block bg-indigo-100 text-indigo-600 rounded-full px-3 py-1 text-xs font-semibold mr-2">{week.videos_count} видео</span>
                  <span className="inline-block bg-gray-100 text-gray-600 rounded-full px-3 py-1 text-xs font-semibold">{new Date(week.start_date).toLocaleDateString()} - {new Date(week.end_date).toLocaleDateString()}</span>
                </div>
                <h2 className="text-xl font-bold mb-2 text-gray-900 truncate">{week.title}</h2>
                <p className="text-gray-600 text-sm line-clamp-3 mb-4">{week.description}</p>
              </div>
